...
def create_app():
    ...
    @app.get('/api/health')
    def health(): return jsonify(success=True, service='Fresh Guard Flask Backend', status='healthy', database=db.mode)
    @app.get('/')
    def home(): return jsonify(success=True, message='Fresh Guard API is running', database=db.mode)
    return app
